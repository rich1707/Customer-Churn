...
subtitle: "Modelling imbalanced data using xgboost"
author: "Richard Ryan"
date: "28 June 2022"
format: 
   html:
...
Obviously the `churn_reason` variable needs to be deleted. It shows `NA` whenever a customer hasn't churned, so to include it would be to make our model trivial: we would (in effect) be using the target variable to predict the target variable. Therefore we can delete:
...
The `churn_score` feature is problematic because it was calculated from a previous model indicating how likely a customer was to churn. As we have no knowledge of how this score was calculated, it is probably best to remove the variable from the dataset:
...
churn |>
   ...
   geom_point(size = 2) +
   geom_segment(
      aes(x = term, xend = term, y = 0, yend = churn_label), 
      linewidth = 1.25
   ) +
...
We can begin by looking at the `latitude` and `longitude` features. In theory, any other feature belonging to the location of the customer should be reflected in this data. So let's group our data by `latitude` and `longitude` and see if there are any obvious patterns:
...
churn |> 
   ...
   summarise(
      ...
      customers = n(),
      ratio = churned / customers
   ) |> 
   ungroup() |> 
   arrange(desc(churned))
```

The numbers of customers who share the same coordinates seem to be no more than four or five, so no real conclusions can be drawn. In which case, let's plot our data and see whether any patterns can be discerned:
...
churn |> 
   ...
   ungroup() |> 
   ggplot(aes(x = longitude, y = latitude, z = ratio)) +
   stat_summary_hex(bins = 30) + 
   labs(
      x = "longitude", 
      y = "latitude",
      title = "Ratio of customers who churn (by location)",
      fill = "%age of \nchurn"
   ) + 
   scale_fill_gradientn(
      colours = c("#E3DE61", "#97B067", "#437057", "#2F5249"),
      labels = percent
   ) +
   theme(
      plot.title = element_text(face = "bold", size = 14),
...
There do appear to be multiple pockets where values of churn are similar---certainly there is enough to include these features in our model.

Is the same true of the other location variables? Let's quickly summarise the city variable:
...
churn |> 
   ...
   pivot_longer(cols = everything(), names_to = "vars", values_to = "values")
```

Here we have over a thousand different cities to consider. A categorical variable with so many levels is always problematical, so we'll need to do some kind of dimensionality reduction to include it in our model.

But is it worth including? To decide this question, it might be helpful to build a simple linear model in which we attempt to predict the value of `churned` by the population of the city in which the customer lives. Therefore we `group_by()` city and use the total number of customers in each city to predict how many people will churn:
...
As such, both city and zip code can be dropped. Any value they have is likely to be caught in the `latitude` and `logitude` features.
...
plot_billing <- churn |> 
   ...
   geom_bar(position = "dodge", width = 0.7, alpha = 0.5) +
...
plot_payment <- churn |> 
   ...
   mutate(payment_method = fct_relevel(
      payment_method, 
      c("Electronic check", "Mailed check", "Bank transfer", "Credit card")
   )) |> 
   ggplot(aes(x = payment_method, fill = churn_label)) + 
   geom_bar(position = "dodge", width = 0.75, alpha = 0.5) + 
...
plot_contract_tech <- churn |> 
   ...
   ggplot(aes(x = contract, fill = churn_label)) + 
   geom_bar(position = "dodge", width = 0.55, alpha = 0.5) +
...
All three remaining plots have the same kind of trajectory, showing that churn decreases as `total_charges` and `tenure` increases. This is largely what we would expect, as people who are long-time customers with very high `total_charges` have repeatedly opted not to churn.
...
The first step is to engineer a couple of features that might help our predictions become more accurate. Our first new variable tells us how the customer's current `monthly_charge` differs from the average `monthly_charge` they have paid previously.
...
With these new features in place we can now start to construct our model.

## Splitting the dataset

Our first task is to split our data into training and test sets. We set a seed so our results are reproducible:
...
churn_recipe <- recipe(churn_label ~ ., data = churn_train) |> 
   ...
   step_smote(churn_label, neighbors = tune(), skip = TRUE)
```
...
Next we `tune()` our model to optimise the above hyper-parameters.

## Tuning our model

To speed things along we shall use all available cores:

```{r}
mirai::daemons(n = 4)
```

Tuning an `xgboost` model can be time consuming even when all CPU cores are being used, so we shall use the `finetune` package as well. This package will stop certain configurations of hyper-parameters early if they are not performing well, which speeds up the `tuning` process quite significantly.

However, two of our hyper-parameters will need updating---`neighbors()` because it is embedded in our `recipe`, and `mtry()` because its upper bound depends upon the number of explanatory variables.

```{r}
param_update <- churn_wrkflw |> 
   extract_parameter_set_dials() |> 
   update(neighbors = neighbors(range = c(2, 12))) |> 
   update(mtry = mtry(range = c(1, 20)))
```

We can now `tune()` our model:
...
```{r}
churn_hypers <- select_best(churn_tune, metric = "accuracy")
```

For reference, here is the best set:
...
So how good is the model we have built?
...
We can end our parallel process at this point:
...
Before evaluating our model's performance let's look at a very naive prediction:
...
It improves significantly on the naive model, to a score of 82.29%; but what we really want to know is how it performs when it comes to predicting customers who churn. When we use the `accuracy` metric we can't really tell. So let's start to address this problem.
...
Consider the values in the *row* marked "Yes". These are all the cases where our model predicted the customer would churn: we get a majority of these predictions right. The `precision` metric measures our model's performance in this respect:
...
So 67.88% of the time we predict a customer will churn we are correct. Note that only 26.6% of the test set did actually churn, so our model is clearly doing useful work.

Now consider the values in the *column* marked "Yes". These are all of the cases where the customer churned, whether we predicted it or not. We now takes the total number of customers who churned and calculate the percentage our model managed to predict. The `recall` metric calculates this for us:
...
So our model manages to predict 63.24% of all cases who churn.

Finally, we can consider the `f_meas` which is the harmonic mean of the precision and recall scores.
...
Here we have a score of 65.48%

# Conclusion

Given we are working with an imbalanced dataset, this is a decent performance on the part of our model. It is often said that it takes four or five times as much spending to attract a customer to a business as it takes to retain an existing customer; so predicting 62.34% of those who churn is a worthwhile result, as measures can be taken to retain the customers mist likely to churn, rather than spending four or five times as much to attract new ones.

The problem here is that we risk spending money to retain customers who in fact had no intention of churning. This is where the `precision` metric is useful. Here our model achieves 67.88%, so not too much money would be spent customers who had no intention of leaving. All in all, our model, while far from perfect, is good enough to add considerable value to a business.
